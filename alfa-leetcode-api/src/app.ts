...
// Add a new endpoint for playground content
app.get('/playground', async (req: express.Request, res: express.Response): Promise<void> => {
    try {
        const playgroundId = req.query.id as string;
        if (!playgroundId) {
            res.status(400).json({ error: 'Playground ID is required' });
            return;
        }
        ...
        await page.goto(`https://leetcode.com/playground/${playgroundId}/shared`, {
            waitUntil: 'networkidle0'
        });

        // Wait for the code content to load
        ...
        res.status(500).json({ error: 'Failed to fetch playground content' });
        return;
    }
});
